    ...
    readdir(SRC_DIR_PATH)
      ...
      .then((dirs) => {
        dirs.forEach((dirName) => {
          readdir(SRC_DIR_PATH + dirName)
            .catch((reason) => {
              console.log(
                `Failed to read a ${SRC_DIR_PATH + dirName} directory.`
              );
              console.log(`${reason}`);
            })
            .then((files) => {
              files.forEach((fileName) => {
                ...
                const SRC_XML_FILE_PATH = SRC_DIR_PATH + dirName + "/";
                ...
                mkdir(DEST_XML_FILE_PATH)
                  .then(() => {
                    console.log(
                      `Suceeded in creating a ${DEST_XML_FILE_PATH} directory.`
                    );
                  })
                  .catch((reason) => {
                    console.log(
                      `Failed to creat a ${DEST_XML_FILE_PATH} directory.`
                    );
                    console.log(`${reason}`);
                  });
